...
export const FruitCard = ({
  ...
  if (!name && !image) {
    return (
      <button
        onClick={onClick}
        className={cn(
          "flex items-center justify-center w-full pb-[133.33%] relative rounded-lg bg-blox-panel transition-all hover:bg-opacity-90",
          className
        )}
      >
        <span className="text-4xl text-white absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">+</span>
      </button>
    );
  }
  ...
  return (
    <div className={cn(
      "rounded-lg overflow-hidden bg-blox-panel group relative pb-[133.33%]",
      className
    )}>
